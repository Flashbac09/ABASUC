            ...
            for (int k = z_start; k <= z_end; ++k)
            {
                ...
                if (i == j)
                {
                    if (sqrt(pow((m * G1.d1 - G1.x[i]), 2) + pow((n * G1.d2 - G1.y[i]), 2) + pow((k * G1.d3 - G1.z[i]), 2)) < G1.rdf_cutoff)
                        res += pow(drdf(distance(m * G1.d1, n * G1.d2, k * G1.d3, G1.x[i], G1.y[i], G1.z[i]), G1.dr), 2) * G1.gd[m * G1.ny * G1.nz + n * G1.nz + k];
                }
                    ...
                    res += drdf(distance(m * G1.d1, n * G1.d2, k * G1.d3, G1.x[i], G1.y[i], G1.z[i]), G1.dr) * drdf(distance(m * G1.d1, n * G1.d2, k * G1.d3, G1.x[j], G1.y[j], G1.z[j]), G1.dr) * G1.gd[m * G1.ny * G1.nz + n * G1.nz + k];
            }
        }
    }
    return res;
